...
**보내기 전 반드시 확인:**
1. `<@` 로 시작하는 멘션이 있는가?
2. 채팅 히스토리에서 다른 에이전트의 멘션 형식을 확인했는가?
3. ID가 올바른 숫자인가?
...
Developer-Claw, Phase 1 시작해.
[HANDOFF] Phase 1 시뮬레이터 구축 시작.
```
...
## Phase 3: ACT 대규모 학습 — 02:00~06:00

**목표**: 1000 에피소드 데이터셋으로 ACT 모델 풀 스케일 학습 (A100 4시간 풀 가동)

### 3a. LeRobot 환경 세팅 + 파일럿 (02:00~02:30)
- A100 VM에 LeRobot 설치/설정 (Phase 2 전송 데이터 확인)
- ACT config 작성:
  - chunk_size: 20
  - batch_size: 64
  - lr: 1e-4
...
- 파일럿: 초기 도착 데이터(~200 에피소드)로 학습 시작, config 검증

### 3b. 풀 스케일 학습 (02:30~05:00)
- 1000 에피소드 전체 데이터셋으로 ACT 학습
- checkpoint 주기: 50 epoch마다
- loss 커브 모니터링 (wandb 또는 텐서보드)
- Phase 2에서 추가 전송되는 에피소드를 점진적으로 반영

### 3c. 평가 + 하이퍼파라미터 튜닝 (05:00~06:00)
- Evaluation 에이전트가 checkpoint별 성능 비교
- 학습된 policy를 시뮬레이터에서 실행 (가능한 경우)
- loss 감소 추세 확인, 필요시 lr/batch_size 조정
- 최종 best checkpoint 선정

**성공 기준**: 1000 에피소드 학습 완료, loss 지속 감소, best checkpoint 확보

**의존**: Phase 2 (LeRobot 데이터 1000 에피소드), A100 VM 정상 동작

**리스크**: Standard 인스턴스로 선점 위험 없음. 비용 관리 주의.

---

## Phase 4: 문서화 + 데모 — 06:00~08:00

### 4a. 결과 정리 (06:00~06:45)
- 각 Phase 성과 요약
- ACT 학습 곡선 캡처 (1000 에피소드 스케일 강조)
- 에피소드 생성 통계 (시나리오 수, 에피소드 수, 데이터 볼륨)
- 성공/실패 분석

### 4b. 데모 영상 (06:45~07:30)
- 로더 레고 치우기 시뮬레이터 실행 영상 (Expert Agent)
- ACT 학습된 policy 실행 영상 (가능한 경우)
- 1000 에피소드 배치 생성 타임랩스
- 5시점 카메라 동시 녹화 데모

### 4c. 다음 단계 (07:30~08:00)
- VLA 확장 계획 (언어 지시 추가)
- 에지케이스 생성기 설계 (강아지, 아이 등 동적 장애물)
- Sim-to-Real 전이 전략 (D2E VAPT)
- 스케일업 로드맵 (1000 → 10000 에피소드)
...
22:00   Phase 2 트리거   #### 시나리오 전달      #### 배치 생성 (목표1000)
22:30   모니터링         #### 추가 시나리오      #### 배치 생성 계속                        대기
23:00   상태 체크        #### 에지케이스 보강    #### LeRobot 변환      대기                #### 품질 검증 시작
23:30   모니터링         .... 계속 생성         #### 변환 + 생성 병행   대기                #### 프레임 검증
00:00   상태 체크        #### 피드백 반영        #### 배치 + 전송       대기                #### 불량 필터링
01:00   모니터링         #### 부족분 보강        #### 1000 에피소드 마감 대기                #### 피드백 → DE
02:00   Phase 3 트리거   대기                  .... 최종 전송         #### A100 세팅       #### 데이터셋 최종검증
02:30   모니터링         대기                  대기                  #### 파일럿 학습      대기
03:00   상태 체크        대기                  대기                  #### 풀스케일 학습    대기
04:00   모니터링         대기                  대기                  #### 학습 중...       대기
05:00   상태 체크        대기                  대기                  #### 평가+튜닝        #### 모델 평가
06:00   Phase 4 트리거   최종 정리              #### 결과 정리         최종 checkpoint      최종 평가
