...
> 향후 시뮬레이터가 확장되면 아래 시나리오를 점진적으로 활성화한다. 현재(Phase 1-3)에서 구현 가능한 항목은 ✅ 표시.

### 환경 변화 (현재 구현 가능)

- ✅ **레고 밀집 배치** — 좁은 영역에 레고 밀집, 경로 계획 복잡
- ✅ **레고 넓은 분산** — 방 전체에 넓게 흩어짐, 효율적 경로 필요
- ✅ **가구 근처 레고** — 가구 가장자리에 레고 배치
- ✅ **다양한 레고 크기** — small/medium/large 비율 변화
- ✅ **레고 개수 극단값** — 5개(쉬움) ~ 50개(어려움)
- ✅ **로더 시작 위치 변화** — 박스와 가까운/먼 시작점
- ✅ **가구 레이아웃 변경** — 3~5가지 프리셋

### 동적 장애물 (향후 Phase 2+)

- 강아지 경로 차단, 레고 물고 감, 따라다님, 박스 뒤엎음
- 아이 놀고 있는 중, 뛰어다님, 정리한 레고 다시 꺼냄, 레고 건네줌
...
### 시나리오 분포 설계 (목표 1,000개)

- 일반 케이스: 500개 (50%) — 바닥 줍기 250, 순차 수거 150, 이동 후 수거 100
- 롱테일 환경: 200개 (20%) — 가구 배치, 밀집, 분산, 크기 변화
- 롱테일 복합: 200개 (20%) — 조립 레고, 유사 물체, 조명
- 롱테일 로봇 실패: 100개 (10%) — 그리핑 실패, 센서 노이즈, 경계 걸림
