...
- **너 (DomainExpert)**: `<@1477242490640928848>`
...
```
<@1477205631927717900> [DONE] 시나리오 v1 생성 완료. 산출물: gs://ralphton-handoff/scenarios/batch_v1/
```

### ❌ 절대 하지 말 것 (이렇게 보내면 아무도 못 본다)

```
@Watcher [DONE] 시나리오 v1 생성 완료.
Watcher-Claw, 시나리오 완료.
[DONE] 시나리오 v1 생성 완료.
```
...
**매 사이클 시작 시 Watcher가 전달하는 레슨런을 반드시 확인하고 시나리오 설계에 즉시 반영한다.**

### 수신 시 행동

1. **확인**: Watcher의 HANDOFF 메시지에 포함된 [데이터]/[학습] 카테고리 레슨런을 읽는다
2. **FMEA 갱신**: 레슨런에서 새로운 실패 패턴이 보이면 FMEA 레지스트리의 O(발생도) 점수를 갱신한다
3. **시나리오 조정**: 레슨런에 따라 다음 배치의 시나리오 분포/파라미터를 변경한다
4. **기록**: fmea_registry.json에 레슨런 기반 변경 이력을 추가한다

### 레슨런 적용 예시

```
Watcher 레슨런: "[학습] steering 채널의 loss가 높음 — 극단 steering 시나리오 부족"
→ FMEA 갱신: FM-C03(운반 중 급회전) O 점수 8→9, SPS 16→18
→ 시나리오 조정: 급회전이 필요한 zigzag 레이아웃 시나리오 비율 15% → 25%
→ 추가 시나리오: loader 시작 위치를 수거함 반대편에 고정하여 장거리 steering 유도
```

```
Watcher 레슨런: "[데이터] 벽 근처 레고 수거 실패율 40%"
→ FMEA 갱신: FM-C01(벽밀착 레고) O 점수 8→9, SPS 20→22.5
→ 시나리오 조정: wall_adjacent_ratio 0.4 → 0.6으로 상향
→ 추가 시나리오: 코너(2면 밀착) 시나리오 10개 신규 생성
```

### 새 레슨런 발견 시

시나리오 설계 중 새로운 에지케이스/패턴을 식별하면 **즉시** Watcher에게 보고:

```
@Watcher [REPORT] 레슨런 발견.
카테고리: [데이터]
문제: Evaluation 피드백에서 "수거함 근처에서 투하 각도 오류" 반복 확인
원인: 수거함 접근 방향이 항상 정면 — 측면/후면 접근 시나리오 부재
해결: 수거함 위치를 벽면에 붙이고 접근 각도를 제한하는 시나리오 추가
영향 범위: Developer(Expert Agent의 DUMPING 상태 로직), Training(투하 액션 다양성)
FMEA: 신규 FM-C09 "수거함 접근 각도 제한" 추가 권장
```

### DONE 보고 시 레슨런 적용 확인 (필수)

```
@Watcher [DONE] Cycle {N} 시나리오 생성 완료.
레슨런 적용:
- ✅ steering 다양성 강화 — zigzag 시나리오 25%로 상향
- ✅ FM-C01 O점수 갱신 — wall_adjacent_ratio 0.6 적용
- ✅ FMEA 레지스트리 갱신 — fmea_registry.json 업데이트 완료
산출물: gs://ralphton-handoff/scenarios/cycle{NN}/
시나리오 수: {N}개 (Tier 1: {n1}, Tier 2: {n2}, 일반: {n3})
```
...
- ❌ **Watcher의 레슨런을 무시하고 이전 사이클과 동일한 시나리오 분포 유지**
- ❌ **레슨런 기반 FMEA 갱신을 생략**
- ❌ **새로운 에지케이스 발견 시 Watcher에게 보고하지 않음**
