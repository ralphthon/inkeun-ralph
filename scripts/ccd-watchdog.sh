#!/bin/bash
# =============================================================================
# Ralphton CCD Watchdog — Claude Code 기반 지능형 감시 시스템
# =============================================================================
# 10분마다 크론으로 실행. Claude Code가 프로젝트 맥락을 읽고,
# 모든 VM과 에이전트 상태를 확인, 문제 해결, 진행 트리거를 자율적으로 수행.
# 3/1 09:00 KST까지 운영.
# =============================================================================
...
# 중첩 세션 방지 환경변수 해제 (cron에서는 불필요하지만 수동 테스트 시 필요)
...
check_deadline() {
  ...
  if [ "$current_date" = "2026-03-01" ] && [ "$current_hour" -ge 9 ]; then
    log "데드라인 도달 (3/1 09:00 KST). CCD Watchdog 종료."
    # 크론잡 자동 제거
    crontab -l 2>/dev/null | grep -v "ccd-watchdog" | crontab - 2>/dev/null || true
    # launchd 자동 제거
    launchctl bootout gui/$(id -u) ~/Library/LaunchAgents/com.ralphton.ccd-watchdog.plist 2>/dev/null || true
    log "스케줄러 자동 제거 완료."
    exit 0
  fi
}
...
acquire_lock() {
  if [ -f "$LOCK_FILE" ]; then
    ...
    if [ -n "$lock_pid" ] && kill -0 "$lock_pid" 2>/dev/null; then
      log "이전 CCD Watchdog 실행 중 (PID: ${lock_pid}). 건너뜀."
      exit 0
    else
      log "이전 lock 파일 정리 (stale PID: ${lock_pid})"
      rm -f "$LOCK_FILE"
    fi
  fi
  echo $$ > "$LOCK_FILE"
...
main() {
  ...
  log "========== CCD Watchdog 실행 시작 =========="

  # 현재 시간 정보 수집
  local current_time
  current_time=$(TZ="Asia/Seoul" date "+%Y-%m-%d %H:%M KST")

  # VM 상태 사전 수집 (Claude에게 전달)
  local vm_status=""
...
PROMPT_EOF
  ...
  log "Claude Code 실행 시작 (model: sonnet)"

  # Claude Code 실행 (sonnet 모델, 비용 절약)
  local output
  output=$("$CLAUDE_BIN" \
    -p \
